...
export function intro(title: string, ...message: string[]) {
  ...
  if (message.length === 0) return stdout.write(`${splittedTitle.map((ttl, index) => `${Colors.FgGreen}${index === 0 ? Symbols.TopLeftCorner : Symbols.LineVertical} ${Colors.Reset + Colors.Reverse} ${ttl} ${Colors.Reset + Colors.FgGreen}`).join('\n')}\n${Symbols.LineVertical}\n${Symbols.LineVertical + Colors.Reset}\n`)
...
export function outro(title: string, ...message: string[]) {
  ...
  if (message.length === 0) return stdout.write(`${splittedTitle.map((ttl, index) => `${Colors.FgGreen}${index === 0 ? Symbols.BottomLeftCorner : ' '} ${Colors.Reset + Colors.Reverse} ${ttl} ${Colors.Reset + Colors.FgGreen}`).join('\n')}${Colors.Reset}\n`)
...
export function info(title: string, ...message: string[]) {
  ...
  if (message.length === 0) return stdout.write(`${splittedTitle.map((ttl, index) => `${Colors.FgGreen}${index === 0 ? Symbols.MiddleLine : Symbols.LineVertical} ${Colors.Reset + Colors.Reverse} ${ttl} ${Colors.Reset + Colors.FgGreen}`).join('\n')}\n${Symbols.LineVertical}\n${Symbols.LineVertical + Colors.Reset}\n`)
